            ...
            egui::ScrollArea::vertical().id_source(scroll_id).auto_shrink([false; 2]).show(ui, |ui| {
                ...
                for (i, line) in display_lines.iter().enumerate() {
                    ...
                    let rich = if let Some(caps) = log_re.captures(line) {
                        ...
                        let is_randomized = formatted.contains("(RandomizedTimer)");
                        let is_auto_timer = formatted.contains("Message generated by automatic timer");
                        let color = if is_randomized || is_auto_timer {
                            egui::Color32::from_rgba_unmultiplied(135, 233, 253, opacity) // #87e9fd
                        } else {
                    ...
                    if let Some(caps) = log_re.captures(line) {
                        ...
                        let is_randomized = msg.contains("(RandomizedTimer)");
                        let is_auto_timer = msg.contains("Message generated by automatic timer");
                        if !msg.trim_start().starts_with("(manual trigger)") && !is_status {
                            ...
                            let msg_color = if is_randomized || is_auto_timer {
                                egui::Color32::from_rgba_unmultiplied(135, 233, 253, opacity) // #87e9fd
                            } else {
                                egui::Color32::from_rgba_unmultiplied(189, 147, 249, opacity) // #9591f9
                            };
                            job.append(
                                msg,
                                0.0,
